...
function App() {
  ...
  const audioRef = useRef({ lastSpoken: '' });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const newFeedback = st.last_feedback || '';
  ...
  const speakInBrowser = (text, force = false) => {
    ...
    if (!force && audioRef.current.lastSpoken === text) return;
    ...
    try {
      ...
      utterance.rate = 1;
      utterance.onend = () => {
        audioRef.current.lastSpoken = text;
      };
      utterance.onerror = () => {
        setAudioError('Audio playback failed. Click Replay Audio or interact with page once.');
      };
      setAudioError('');
      setTimeout(() => window.speechSynthesis.speak(utterance), 80);
    } catch {
      setAudioError('Unable to play audio feedback in browser.');
    }
  };
  ...
  const onVoiceResult = (payload) => {
    ...
    speakInBrowser(newFeedback, true);
  };
    ...
    <div className={`app-shell ${theme}`}>
      <div className="container py-4">
        ...
        <div className="status-row mb-3">
          ...
          <span className="badge text-bg-info">Detected At: {updatedAt ? new Date(updatedAt * 1000).toLocaleTimeString() : 'N/A'}</span>
        </div>

        {backendError && <div className="alert alert-warning py-2">{backendError}</div>}
        {audioError && <div className="alert alert-info py-2">{audioError}</div>}
        ...
        <div className="row g-3">
          ...
          <div className="col-lg-4 d-grid gap-3">
            ...
            <NavigationPanel feedback={feedback} feedbackAt={feedbackAt} onReplay={() => speakInBrowser(feedback, true)} />
